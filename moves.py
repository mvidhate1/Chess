...
def matrix_to_tuple(array, empty_array):
    """
    Given a matrix, converts it to 2D tuple. 
    Useful for using a matrix as a key in a dictionary
...
def special_move_gen(board,color,moves = None):
    '''
    Uses board state and a color, returns a move dict with the possible
    special moves. Only returns castling moves as pawn promotion is
...
def move_gen(board, color, attc = False):
    ...
    Does not check to see if the move puts you in check, that is done
...
def minimax(board, depth, alpha, beta, maximizing, memo):
    """
    Minimax algorithm with alpha-beta pruning to determine the best move for
    AI from the current board state.
    """

    # convert the 2D list to a tuple, so it can be used as a key in mem
