...
def check_promotion(piece,y):
    '''
    Checks pawn promotion eligibility on it's next move.
    '''
    if piece.color == "w":
        row = 1
        inc = -1
...
class Board:
    """
    Board is represented by an 8x8 array
    ...
    def __init__(self, color):
        ...
        if color == 1:
            ...
            self.array = [
                [self.black_rook_left, Knight("b", 0, 1), Bishop("b", 0, 2), Queen("b", 0, 3), self.black_king, Bishop("b", 0, 5), Knight("b", 0, 6), self.black_rook_right],
                ...
                [self.white_rook_left, Knight("w", 7, 1), Bishop("w", 7, 2), Queen("w", 7, 3), self.white_king, Bishop("w", 7, 5), Knight("w", 7, 6), self.white_rook_right]
            ]
        # # # you are black
        # elif color == 2:
        #     self.empty = [[None for x in range(8)] for y in range(8)]
        #     self.black_king = King("b", 7, 4) # these objects are stored for easy
        #     self.white_king = King("w", 0, 4) # access for special move checking
        #     self.white_rook_left = Rook("w", 0, 0)
        #     self.white_rook_right =  Rook("w", 0, 7)
        #     self.black_rook_left = Rook("b", 7, 0)
        #     self.black_rook_right = Rook("b", 7, 7)
        #     self.array = [
        #         [self.white_rook_left, Knight("w", 0, 1), Bishop("w", 0, 2), Queen("w", 0, 3), self.white_king, Bishop("w", 0, 5), Knight("w", 0, 6), self.white_rook_right]
        #         [Pawn("w", 1, i) for i in range(8)],
        #         [None for x in range(8)],
        #         [None for x in range(8)],
        #         [None for x in range(8)],
        #         [None for x in range(8)],
        #         [Pawn("b", 6, i) for i in range(8)],
        #         [self.black_rook_left, Knight("b", 7, 1), Bishop("b", 7, 2), Queen("b", 7, 3), self.black_king, Bishop("b", 7, 5), Knight("b", 7, 6), self.black_rook_right],
        #     ]

            # used in minimax to find the best move
            self.score = 0
            # maps piecetype to relative value
            self.pvalue_dict = {King: 200, Queen: 10, Rook: 5, Knight: 3, Bishop: 3, Pawn: 1}
